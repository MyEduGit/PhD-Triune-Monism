...
# Page chrome
# ---------------------------------------------------------------------------
st.set_page_config(page_title="Claw Fleet Dashboard", page_icon="🦞", layout="wide")
...
# ---------------------------------------------------------------------------
# Helpers (defined early so Streamlit top-to-bottom reruns find them)
# ---------------------------------------------------------------------------
def notify(email: bool, telegram: bool, code: int):
    ...
    if telegram:
        ...
        try:
            requests.post(
                ...
                data={"chat_id": chat, "text": f"NanoClaw pipeline complete. exit={code}"},
                timeout=10,
            )
...
def load_report():
    ...
    return {
        ...
        "hypotheses": [
            {
                "Conflict_Source": {"Subject": "Triune Monism", "Predicate": "grounds"},
                "Hypothesis": "Placeholder hypothesis.",
                "Missing_Data/Theory":
                    "Run ee_extract_triples.py + sre_process_and_generate.py.",
                "Suggested_Test":
                    "Click 'Run Full Pipeline' below once scripts are in place.",
            }
        ],
    }, "placeholder"
...
with st.sidebar:
    ...
    for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "XAI_API_KEY"):
        tag = '<span class="live">SET</span>' if os.getenv(k) else '<span class="miss">MISSING</span>'
...
def call_claude(task: str) -> str:
    ...
    try:
        r = requests.post(
            ...
            json={"model": ep["anthropic_model"], "max_tokens": 1024,
                  "messages": [{"role": "user", "content": task}]},
            timeout=120,
        )
        r.raise_for_status()
...
def dispatch_single(name: str, task: str):
    if not task.strip():
...
tag_class = {"live": "live", "demo": "pend", "placeholder": "miss"}[origin]
st.markdown(
    f'Data source: <span class="{tag_class}">{origin.upper()}</span> '
...
# ---------------------------------------------------------------------------
st.header("3. Orchestrator — zero-touch pipeline")
...
with run1:
    if st.button("▶ Run launcher.sh (EE → CM/HG → dashboard)",
                 use_container_width=True, disabled=not launcher_present):
        with st.spinner("Pipeline running…"):
            p = subprocess.run(["bash", str(LAUNCHER)],
                               cwd=ROOT, capture_output=True, text=True, timeout=900)
        st.success(f"exit={p.returncode}")
        st.text_area("stdout", p.stdout, height=240)
        st.text_area("stderr", p.stderr, height=120)
        notify(notify_email, notify_telegram, p.returncode)
with run2:
    if st.button("▶ Run NanoClaw_Orchestrator.py",
                 use_container_width=True, disabled=not orchestrator_present):
...
with st.expander("ℹ️ Ground truth & receipts"):
    ...
    st.markdown(f"- **Report path**: `{JSON_PATH}` "
                f"({'exists' if JSON_PATH.exists() else 'missing'})")
    st.markdown(f"- **Orchestrator**: `{ORCHESTRATOR}` "
                f"({'exists' if ORCHESTRATOR.exists() else 'missing'})")
    st.markdown(f"- **Launcher**: `{LAUNCHER}` "
                f"({'exists' if LAUNCHER.exists() else 'missing'})")
    st.markdown(
        "- **Dispatch targets**: "
